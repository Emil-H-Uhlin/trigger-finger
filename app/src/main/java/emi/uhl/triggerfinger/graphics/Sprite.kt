...
open class Sprite(var bitmap: Bitmap?,
    ...
    val origin get() = if (bitmap == null) Vector2.zero else Vector2(bitmap!!.width / 2f, bitmap!!.height / 2f) * scale
    private val degrees get() = Math.toDegrees(transform.rotation.toDouble()).toFloat() % 360
    
    override fun draw(canvas: Canvas, paint: Paint?) = if (bitmap != null) {
